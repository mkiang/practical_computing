...
:::
::: {.column width="35%"}
![](assets/xkcd_2054_data_pipeline.png){fig-align="center" width="95%"}
:::
::::
...
::: {.notes}
~2 min. Every item on this list will seem obvious by Week 3. Ask students which of these they have encountered.
:::
...
::: aside
Sandve et al. (2013). [Ten Simple Rules for Reproducible Computational Research](https://doi.org/10.1371/journal.pcbi.1003285). *PLOS Comp Bio*, 9(10): e1003285.
:::

::: {.notes}
~1 min. The three rules from Sandve et al. are a good litmus test for any analysis project. We will return to these throughout the course.
:::

## The research computing lifecycle

![The research computing lifecycle.](assets/research_lifecycle.svg){fig-align="center" height="475"}

::: {.notes}
~2 min. Walk through the diagram. Each stage produces artifacts (code, data, documentation) that together constitute the research compendium. The manuscript is just one output — every stage generates decisions and outputs that need recording.
:::
...
::: {.notes}
~3 min. Students often learn this pattern from tutorials. It gives a false sense of security. If the script depends on a package loaded interactively but not called with `library()`, `rm(list = ls())` won't catch it.
:::

## The only reliable clean slate — restart R {.smaller}
...
::: {.notes}
~2 min. Have students change their RStudio settings right now. This ensures every session starts fresh — no hidden state carried over.
:::
...
![Modular pipeline.](assets/modular_pipeline.svg){fig-align="center" height="500"}
...
![Path resolution across machines.](assets/here_resolution.svg){fig-align="center" height="425"}

::: {.notes}
Appendix slide. Use if time permits or if students ask how here::here() resolves paths across machines. Alice's Mac resolves to `/Users/alice/projects/mortality_analysis/data_raw/raw_deaths.csv`. Bob's Windows PC resolves to `C:/Users/bob/Documents/mortality_analysis/data_raw/raw_deaths.csv`. The code is identical — the `.Rproj` file handles the rest. The lecture notes have a deep dive on the underlying rprojroot algorithm.
:::
